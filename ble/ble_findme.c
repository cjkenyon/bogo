    ...
    switch(event)
    {
        ...
        case CY_BLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ:
        {
            ...
            if (CY_BLE_BUTTONS_USR_BTN_CHAR_HANDLE == read_req_param->attrHandle)
            {
                ...
                printf("[INFO] : reading proximity sensor\r\n");
                //bool prox_rslt = prox_read();
                //if (prox_rslt) {
                //    PROX_VALUE = 1;
                //} else {
                //    PROX_VALUE = 0;
                //}
                PROX_VALUE = prox_read0();
                CY_BLE_GATT_DB_ATTR_SET_GEN_VALUE(CY_BLE_BUTTONS_USR_BTN_CHAR_HANDLE, &PROX_VALUE, 1);
                printf("[INFO] : PROX_VALUE %d\r\n", PROX_VALUE);

            }
            if ( CY_BLE_TEMP_USR_TEMP_CHAR_HANDLE == read_req_param->attrHandle) {

                printf("[INFO] : trying to read from USR_TEMP characterisitc\r\n");

                LM75_get_temp();
                CY_BLE_GATT_DB_ATTR_SET_GEN_VALUE(CY_BLE_TEMP_USR_TEMP_CHAR_HANDLE,&TEMP_VALUE,1);

                printf("[INFO] : TMP_VALUE %d\r\n", TEMP_VALUE);
            }
        ...
        case CY_BLE_EVT_GATTS_WRITE_CMD_REQ:
        		{
        			...
        			if( CY_BLE_LEDS_USR_LED_CHAR_HANDLE == write_req_param->handleValPair.attrHandle)
        			{
                        ...
                        lights(write_req_param->handleValPair.value.val[0]);

        			}
                    if ( CY_BLE_MOTOR_MOTOR_CHAR_HANDLE == write_req_param->handleValPair.attrHandle) 
                    {
                        
        				printf("[INFO] : GATT write MOTOR characteristic with value: 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                        drive_dir(write_req_param->handleValPair.value.val[0]);
                        
                    }
                    if ( CY_BLE_PUMP_USR_PUMP_CHAR_HANDLE == write_req_param->handleValPair.attrHandle) 
                    {
                        printf("[INFO] : GATT write PUMP characteristic with value 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                        printf("Recieved characteristic\r\n");
                        pump(write_req_param->handleValPair.value.val[0]);
                        if (write_req_param->handleValPair.value.val[0] == 0x30) {
                            buzz(false);
                            lights(false);
                        } else if(write_req_param->handleValPair.value.val[0] == 0x31) {
                            buzz(true);
                            lights(true);
                        }
                    ...
                    if ( CY_BLE_BUZZER_USR_BUZZER_CHAR_HANDLE == write_req_param->handleValPair.attrHandle)
                    {
                        printf("[INFO] : GATT write BUZZER characterristic with value 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                    }
        		}
        default:
        {
